...
import Vue3SketchRuler from 'vue3-sketch-ruler';
...
// 滚动条拖动的宽度
const containerWidth = computed(() => `${window.innerWidth * 2}px`);

// 滚动条拖动的高度
const containerHeight = computed(() => `${height.value * 2}px`);
...
// 计算画布大小
const canvasBox = () => {
  const layoutDom = document.querySelector('#go-chart-edit-layout');
  if (layoutDom) {
    // 此处减去滚动条的宽度和高度
    const scrollW = 20;
    return {
...
<template>
  <div class="editor-RendererContainer relative size-full of-hidden">
    <Vue3SketchRuler
